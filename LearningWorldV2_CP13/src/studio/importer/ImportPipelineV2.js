 ...
 *   Generator → Transaction → Commit → Manifest → Report
 */
export class ImportPipelineV2 {
  ...
  /**
   * Parser may be injected later via setParser().
   * This allows flexible configuration before pipeline execution.
   */
  ...
  async #parse(input, report) {
    ...
    report.sourceFormat = parsed.format || 'unknown';
  ...
  async #generate(normalized, report) {
    if (!this.#generator) {
      throw new Error('No generator set');
    }
  ...
  async #updateManifest(normalized, report) {
    ...
    // TODO Sprint 14: Implement ManifestBuilder
    void normalized;
    void report;
  }
}